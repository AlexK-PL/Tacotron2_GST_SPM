...
class DataPreparation(torch.utils.data.Dataset):
    ...
    def melspec_textSequence_pair(self, audiopath_and_text):
        ...
        prosody_tensor_path = self.prosody_features_path + wav_name + "_dense_pitch_info.pt"
        gst_prosody_tensor = torch.FloatTensor(torch.load(prosody_tensor_path))
        pitch_contour = gst_prosody_tensor[0, :].type(torch.IntTensor)

        pitch_hot_matrix = torch.IntTensor(gst_prosody_tensor.size(1), tacotron_params['n_mel_channels'])
        pitch_hot_matrix.zero_()

        for j in range(len(pitch_contour)):  # tengo que recorrer todos los frames
            bin = int(pitch_contour[j])
            if bin > 0:
                pitch_hot_matrix[j, pitch_contour[j]] = 1

        pitch_hot_matrix_suband = pitch_hot_matrix[:, 1:14]
        pitch_hot_matrix_suband = pitch_hot_matrix_suband.transpose(0, 1)
        ...
        return sentence_sequence, mel_spec, pitch_hot_matrix_suband
